...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
    ...
    float predictedPitch = attitude_update.Pitch();    // pitchEst + dtIMU * gyro.y;
    float predictedRoll = attitude_update.Roll();        // rollEst + dtIMU * gyro.x;
    ekfState(6) = attitude_update.Yaw();                // ekfState(6) + dtIMU * gyro.z;    // yaw
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
    ...
    predictedState(0) = curState(0) + dt * curState(3);                    // predict pos_x
    predictedState(1) = curState(1) + dt * curState(4);                    // predict pos_y
    predictedState(2) = curState(2) + dt * curState(5);                    // predict pos_z
    
    V3F accel_global = attitude.Rotate_BtoI(accel);
    predictedState(3) = curState(3) + dt * accel_global.x;                // predict vel_x
    predictedState(4) = curState(4) + dt * accel_global.y;                // predict vel_y
    predictedState(5) = curState(5) + dt * (accel_global.z - 9.81f);        // predict vel_z
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
    ...
    zFromX(0) = ekfState(6);
    float yaw_diff = magYaw - zFromX(0);
